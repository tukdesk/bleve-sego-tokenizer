...
// func TestSegoTokenizerDefaultModeWithCaseSensitive(t *testing.T) {
// 	tests := []struct {
// 		input  []byte
...
// 	tokenizer, _ := NewSegoTokenizer("dict.txt", false, true)
...
func TestSegoTokenizerSearchModeWithCaseSensitive(t *testing.T) {
	tests := []struct {
		input  []byte
		output analysis.TokenStream
	}{
	...
	tokenizer, _ := NewSegoTokenizer("dict.txt", true, true)
	for _, test := range tests {
		actual := tokenizer.Tokenize(test.input)
		if !reflect.DeepEqual(actual, test.output) {
